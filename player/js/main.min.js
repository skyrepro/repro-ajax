! function() {
    ...
    _.forEach(_ => {
        ! function _(e) {
            ...
            e.addEventListener("click", () => {
                e.classList.toggle($), t.classList.toggle($)
            });
            let r = _ => {
                t.contains(_.target) || e.contains(_.target) || (e.classList.remove($), t.classList.remove($))
            };
            document.addEventListener("click", r);
            let l = t.querySelector("[data-close]");
            l && (l.onclick = function() {
                e.classList.remove($), t.classList.remove($)
            })
        }(_)
    });
    let e = {},
        ...
        r = async (_, $, t, r, l = "spotify") => {
            ...
            let a = s.toLowerCase();
            if (e[a]) return e[a];
            let o = `https://api.streamafrica.net/new.search.php?query=${encodeURIComponent(s)}&service=${l.toLowerCase()}`,
                i = await fetch(o);
            if (403 === i.status) {
                let n = {
                    title: $,
            ...
            if (!C.results || 0 === C.results.length) {
                let c = {
                    title: $,
                    artist: _,
                    art: t,
            ...
            if (!i.results || 0 === i.results.length) return {
                title: $,
                artist: _,
                art: t,
                cover: r
    ...
    function i(_, $ = null) {
        $ && (_.src = $), _.load(), _.play(), a.innerHTML = t.pause, a.classList.add("is-active"), document.body.classList.add("is-playing")
    }

    function n(_) {
        _.pause(), a.innerHTML = t.play, a.classList.remove("is-active"), document.body.classList.remove("is-playing")
    }
    ...
    function m(_) {
        C.value = _, h(_), y(_), localStorage.setItem("volume", _), o.volume = _ / 100
    }
    ...
    null !== C && (m(v), C.addEventListener("input", _ => {
        m(_.target.value)
    ...
    let g = new ResizeObserver(() => {
        c.removeAttribute("style"), L.removeAttribute("style"), p = u.offsetWidth < 100;
        let _ = C.value;
        h(_), y(_)
    });
    g.observe(u), null !== d && d.addEventListener("click", () => {
        o.volume > 0 ? (m(0), d.innerHTML = t.mute) : (m(100), d.innerHTML = t.volumeUp)
    });
    ...
    x.addEventListener("click", () => {
        k.classList.toggle("is-fullscreen"), w.classList.toggle("is-fullscreen"), x.classList.toggle("is-active")
    }), ! function _() {
        let $, e, r = window.streams || {},
            l = r.station;
        o.src = l.stream_url, l.social && M && M.forEach(_ => {
            Object.keys(l.social).forEach($ => {
                _.appendChild(function _($, e) {
                    let r = document.createElement("a");
                    return r.classList.add("player-social-item"), r.href = $, r.target = "_blank", r.innerHTML = t[e], r
                }(l.social[$], $))
            })
        }), ! function _() {
            $ && clearTimeout($);
            ...
            fetch(t).then(_ => _.json()).then(async _ => {
                ...
                if (e !== $) {
                    ...
                    (function _($) {
                        ...
                        } else Z.classList.remove("is-scrolling"), Z.removeAttribute("style");
                        let t = k.querySelector(".player-artwork");
                        if (t) {
                            let r = document.createElement("img");
                            r.src = $.art, r.width = 600, r.height = 600, r.addEventListener("load", () => {
                                t.appendChild(r), setTimeout(() => {
                                    t.querySelectorAll("img").forEach(_ => {
                                        _.style.transform = `translateX(${-_.width}px)`, _.addEventListener("transitionend", () => {
                                            t.querySelectorAll("img:not(:last-child)").forEach(_ => {
                                                _.remove()
                                            }), _.style.transition = "none", _.style.transform = "none", setTimeout(() => {
                        ...
                        if (H || q) {
                            let l = new Image;
                            l.src = $.cover || $.art, l.addEventListener("load", () => {
                                H.style.opacity = 0, q.style.opacity = 0, H.addEventListener("transitionend", () => {
                                    H.src = $.cover || $.art, q.src = $.cover || $.art, H.style.opacity = 1, q.style.opacity = 1
                                })
                            })
                        }
                    })(o),
            ...
            }).catch(_ => console.log(_)), $ = setTimeout(_, b)
        }()
    }()
}();